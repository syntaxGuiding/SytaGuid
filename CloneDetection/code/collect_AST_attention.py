...
checkpoint_prefix = "saved_models/model_DD.bin"

model.load_state_dict(torch.load(checkpoint_prefix), strict=False)
...
syntax_attention_weights = getSyntaxAttentionScore(model, data[:4000], tokenizer, syntax_list)
import pickle
# pickle the result
with open('DD4CD_syntax_attention_weights.pkl', 'wb') as f:
